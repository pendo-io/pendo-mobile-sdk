...
Entered from `SKILL.md` Phase 2 (read once, in full) and executed from Phase 6, with `apiKey`, `urlScheme`, `platform="android"`, `subPlatform` (`kotlin` | `java`), and `branchName` already resolved and in scope — never re-ask for them, never re-run detection. Wherever code below shows `"<apiKey>"`, `"<urlScheme>"`, or `<currentMinor>` (quoted, angle-bracket — `<currentMinor>` appears inside a larger quoted string, e.g. `"<currentMinor>.+"`), that is a string-literal template slot: write the real, already-resolved value as the literal string content; never write the literal angle-bracket token into a file. `apiKey`/`urlScheme` are resolved by `SKILL.md` Phase 5 before this file is ever entered; `currentMinor` is resolved here, in §2, since it is Android-specific.
...
dependencies {
    implementation group: 'sdk.pendo.io', name: 'pendoIO', version: '<currentMinor>.+'
}
```
...
Pendo's own snippet (`android/pnddocs/native-android.md`) also carries `changing:true`/`isChanging = true` alongside the dynamic selector, but that flag is Gradle's mechanism for a *fixed* version whose already-published bytes can later change in place — a different caching concern (`cacheChangingModulesFor`) that a dynamic `+` selector doesn't need, so it is a no-op here and intentionally omitted.
...
**Optional — scope to debug builds only.** If exporting this activity in production builds is undesirable, move this `<activity>` block into a debug-only manifest (`src/debug/AndroidManifest.xml`) instead of the main one — Android's build-variant manifest merging then includes it only in debug builds. Trade-off: Pendo Designer pairing and tagging then only work against debug builds, not release or staging builds shipped to testers. This isn't Pendo's own documented default and isn't part of the baseline install — only do it if asked.
