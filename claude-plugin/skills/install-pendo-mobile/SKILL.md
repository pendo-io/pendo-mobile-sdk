...
2. *(Skipped under `--dry-run`.)* Capture where to return to, as `originalRef`. Run `git branch --show-current`; if it prints a name, that is `originalRef`. **If it prints nothing, the repo is on a detached HEAD** — a tag checkout, a bisect, a CI or submodule checkout — and there is no branch name to record. Fall back to `git rev-parse HEAD` and record that commit SHA as `originalRef` instead, noting that it is a commit rather than a branch. Never treat the empty string as a branch name: `git checkout ""` does not return anywhere, and the Phase 8 report would render an empty backtick pair as if a branch existed. Then create the working branch: `git checkout -b pendo-install-<platform>` (`platform` has been known since Phase 1, so there's no generic-name placeholder to resolve later). **If the skill stops for any reason between this point and Phase 8's success report, discard everything this run wrote before switching back — `git -C "$(git rev-parse --show-toplevel)" checkout -- .` to revert any tracked-file edits, then `git -C "$(git rev-parse --show-toplevel)" clean -fd` to remove new files or directories this run created, both anchored at the git root (each acts only on the current directory downward, and Phase 7 may have left the shell in a monorepo's app root) and while still on the install branch — then switch to `originalRef` and `git branch -d pendo-install-<platform>`** so no empty branch is left behind, and say so in the report — see Phase 8's early-exit report. `git checkout <originalRef>` alone is not a rollback: nothing in this skill ever commits Phase 6's edits to the install branch, so with uncommitted changes present, git either carries them onto `originalRef`'s working tree or refuses the checkout outright. Checking out a recorded SHA restores the same detached HEAD the run started from, which is the correct no-op — but only once the working tree is already clean.
...
If a branch was created in Phase 5 and the run then stops before this success report, discard the run's own edits from the git root (`git -C "$(git rev-parse --show-toplevel)" checkout -- .` then `git -C "$(git rev-parse --show-toplevel)" clean -fd`, on the install branch), run `git checkout <originalRef>`, then `git branch -d pendo-install-<platform>`, and reflect that switch-back in this template's `Branch` line.
...
- If the skill stops after Phase 5's branch was created, always discard the run's own edits from the git root (`git -C "$(git rev-parse --show-toplevel)" checkout -- .`, then `... clean -fd`) before switching back to `originalRef`, delete the empty install branch afterwards, and say so in the Phase 8 report — never leave the repo on a stray branch, never leave one behind, and never leave the run's edits stranded on `originalRef`, after an aborted run. On a detached HEAD, `originalRef` is the commit SHA; never treat an empty `git branch --show-current` as a branch name.
