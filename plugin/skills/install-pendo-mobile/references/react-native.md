...
**Module-level placement alone only orders code within the same file.** If the navigation wrapper (`WithPendoReactNavigation(...)`, etc.) is declared in a different module than `setup()` — e.g. a separate `AppNavigator.tsx` that the entry file imports — static imports evaluate the imported module's top-level code *before* the importing file's own body runs, regardless of where the two calls appear in their respective source files. When the wrapper lives in another module, put the `setup()` call in its own module (e.g. `pendo-init.ts`) and make it the **first import statement** in the entry file, before any import that could transitively reach the navigation wrapper. Import order is evaluation order — this holds even when the wrapper is several modules away.
